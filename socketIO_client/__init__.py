...
class BaseNamespace(object):
    ...
    def on_event(self, event, *args):
        ...
        log.info('%s [event] %s(%s)', self.path, event, ', '.join(arguments))

    def on_error(self, reason, advice):
        """Called after server sends an error; you can override this method"""
        log.info('%s [error] %s', self.path, advice)

    def on_noop(self):
        """Called after server sends a noop; you can override this method"""
        log.info('%s [noop]', self.path)

    def on_open(self, *args):
        log.info('%s [open] %s', self.path, args)

    def on_close(self, *args):
        log.info('%s [close] %s', self.path, args)

    def on_retry(self, *args):
        log.info('%s [retry] %s', self.path, args)

    def on_reconnect(self, *args):
        log.info('%s [reconnect] %s', self.path, args)
...
class SocketIO(object):
    ...
    def _get_transport(self):
        ...
        log.debug('[transports available] %s', ' '.join(
